...
export async function POST(req: Request) {
  ...
  const tokenPayload = { user: { _id: user._id } }
  ...
  return new NextResponse('Logged Successful', {
    status: 200,
    headers: {
      'Set-Cookie': `token=${token}; HttpOnly; Max-Age=${process.env.SESSION_EXPIRES}; Path=/; SameSite=None; Secure`,
    },
  })
}
