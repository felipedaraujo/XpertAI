  ...
  if (isLogged && data) {
    return (
      <div className={styles.settings}>
        ...
        {isModal &&
          SettingsList.filter((content) => content.title == modal).map(
            (modal, index) => (
              <SettingsModal
                key={index}
                email={data.email}
        ...
        <div className={styles.settings__profile}>
          ...
          <div className={styles.settings__profile__section}>
            <p className={styles.settings__profile__section__title}>
              Ai Prompts
            </p>
            <LinearProgress
              variant="determinate"
            ...
            <p className={styles.settings__profile__section__prompts}>
              <span style={{ fontWeight: 'bold' }}>{data.prompts}</span> /{' '}
              {data.plan == 'buisness'
                ? 20000
                : data.plan == 'free'
                ? 1000
          ...
          <hr />
          <div className={styles.settings__profile__section}>
            <p className={styles.settings__profile__section__delete__btn}>
              Delete Account
            </p>
